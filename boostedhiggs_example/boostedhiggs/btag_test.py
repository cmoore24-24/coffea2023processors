...
class BTagCorrector:
    def __init__(self, year, workingpoint):
        ...
        files = {
            ...
            '2018': '/cvmfs/cms.cern.ch/rsync/cms-nanoAOD/jsonpog-integration/POG/BTV/2017_UL/btagging.json.gz',
        }
        filename = os.path.join(os.path.dirname(__file__), 'data', files[year])
        btag = correctionlib.CorrectionSet.from_file(filename)
        print(btag)
        sf = btag["deepJet_incl"].evaluate("central", "M", awkward.Array(events.Jet.hadronFlavour), awkward.Array(abs(events.Jet.eta)), awkward.Array(events.Jet.pt))
        abcd
